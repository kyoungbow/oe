...
<body>
    ...
    <div class="signup">
        ...
        <div class="signup-container">
            <div>
                <h1>My Fish(id, pw, check, name);</h1>
            </div>
            <div class="signup-content">
                ...
                <div>
                    <input id="id" type="text" name="id" value="${memberInfo.id}" readonly>
                </div>
                <div>
                ...
                <div>
                    <input id="email" name="email" type="text" value="${memberInfo.email}" readonly">
                    <c:if test="${memberInfo.auth == 0}">
                    <button style="border-radius:3px" type="button" id="btnEmail">이메일 인증</button>
                    </c:if>
                    <c:if test="${memberInfo.auth == 1}">
                    <button style="border-radius:3px" type="button" disabled>인증된 이메일</button>
                    </c:if>
                ...
                <button type="button" class="mb-3 btn btn-sm btn-secondary" id="btnSearchAddr">주소검색</button>
                    <div class="col-md-4">
                    	<input class="form-control" id="roadAddr" name="roadAddr" value="${memberInfo.roadAddr}" readonly />
                    </div>
	                <div class="mb-3">
	                    <input class="form-control" id="addrDetail" name="addrDetail" value="${memberInfo.addrDetail}" readonly />
	                </div>
				...
				<input type="hidden" name="roadFullAddr" id="roadFullAddr" value="${memberInfo.roadAddr}">
				<input type="hidden" name="jibunAddr" id="jibunAddr" value="${memberInfo.jibunAddr}">
                <div class="signup-button">
                    <button>Modify</button>
                    <button formaction="secession" onclick="return confirm('삭제하시겠습니까?')">Remove</button>
                </div>
            </div>
